...
# ---------------------------------------------------------------------------
# conversation_root integration tests (real TelemetryHandler + real spans)
# ---------------------------------------------------------------------------


def _make_real_handler():
    """Return a handler wired to a real TelemetryHandler with an in-memory exporter."""
    ...
    telemetry = TelemetryHandler(tracer_provider=tracer_provider)
    handler = OpenTelemetryLangChainCallbackHandler(telemetry)
    return handler, exporter


class TestConversationRootIntegration:
    """Integration tests that verify gen_ai.conversation_root is emitted correctly."""

    def test_root_agent_has_conversation_root_on_span(self):
        """Root agent (parent_run_id=None) gets gen_ai.conversation_root=True."""
        handler, exporter = _make_real_handler()
        run_id = _run_id()

        handler.on_chain_start(
            serialized={"name": "travel_planner"},
            inputs={},
            run_id=run_id,
            parent_run_id=None,
            metadata={"agent_name": "Travel Planner"},
        )
        handler.on_chain_end(outputs={}, run_id=run_id)
    ...
    def test_child_agent_lacks_conversation_root_on_span(self):
        """Child agent (has a parent invocation) does NOT get gen_ai.conversation_root."""
        handler, exporter = _make_real_handler()
        parent_run_id = _run_id()
        child_run_id = _run_id()
        ...
        # Start child agent nested inside the root
        handler.on_chain_start(
            serialized={"name": "flight_search"},
            inputs={},
            run_id=child_run_id,
            parent_run_id=parent_run_id,
            metadata={"agent_name": "Flight Search Agent"},
        )
        ...
        spans = exporter.get_finished_spans()
        root_span = next(
            s for s in spans if "Travel Planner" in s.name
        )
        child_span = next(
            s for s in spans if "Flight Search Agent" in s.name
        )
        assert root_span.attributes.get("gen_ai.conversation_root") is True
        assert "gen_ai.conversation_root" not in child_span.attributes

    def test_root_workflow_has_conversation_root_on_span(self):
        """Root workflow (LangGraph, parent_run_id=None) gets gen_ai.conversation_root=True."""
        handler, exporter = _make_real_handler()
        run_id = _run_id()
