...
class TelemetryHandlerWorkflowConversationRootTest(_WorkflowTestBase):
    # ------------------------------------------------------------------
    # conversation_root auto-detection
    # ------------------------------------------------------------------

    def test_root_workflow_gets_conversation_root_true(self) -> None:
        """Workflow with no active parent span is auto-marked as conversation root."""
    ...
    def test_child_workflow_does_not_get_conversation_root(self) -> None:
        """Workflow created inside an active parent span is NOT marked as root."""
        with self.handler.workflow(name="parent_wf") as parent:
    ...
    def test_conversation_root_not_emitted_when_none(self) -> None:
        """When conversation_root is None, attribute is not emitted on the span."""
        with self.handler.workflow(name="parent") as _parent:
            child = self.handler.workflow(name="child")
            # conversation_root is None for child (has active parent)
            self.assertIsNone(child.conversation_root)
            child.stop()
